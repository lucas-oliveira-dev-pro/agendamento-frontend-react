...
export default function Pendentes() {
  ...
  async function load() {
    try {
      ...
      setItems(
        list.filter((item) => item.status !== "cancelado" && !item.paid),
      );
      setTotal(
        list
          .filter((item) => item.status !== "cancelado" && !item.paid)
          .reduce((sum, item) => sum + Number(item.value_cents || 0), 0),
      );
    } catch (e) {
      setError(
  ...
  return (
    <Box>
      ...
      <Typography color="text.secondary" mb={2}>
        Todos os agendamentos ainda não pagos.
      </Typography>

      <Paper style={{ padding: "1rem" }} elevation={0} className="summary-card">
